...
- [ ] **前台優化執行（Codex）— 依 `docs/FRONTEND-REVIEW-2026-07-16.md` 排序**（四視角綜合審查，已含業主定案的收費/保固口徑）。建議節奏＝先 P0 快贏 → P1 信任地基 → P2 內容對等 → P3 精修：
  - **P0 立即快贏（低成本高成效）**：① 預約表單「地址」降為選填/市區級、步進子表單預設收合（`header.js`）② 手機補「一鍵直達 LINE」雙鈕（現在底部列點下去是開表單）③ 首屏改雙 CTA（加 LINE 免費初判＋填表）④ 修手機 header 吃首屏＋spacer 130→~168px 消跳動 ⑤ 服務頁主 CTA/價格改吃專屬色＋修 `craft.css` 首頁六 icon 只給 4 色的掉色 bug
