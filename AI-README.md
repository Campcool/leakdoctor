...
最後更新：2026-07-12（by Codex）— 真機 Header、懸浮 LINE、返回鍵與冷氣前後圖修正版已部署
...
- **`header.js` 是全站共用核心**（每頁 `<script src="header.js">` 或 `../header.js` 載入），runtime 注入：
  - sticky header＋4 個主服務頁籤（root 絕對路徑 `/xxx.html`，讓 /articles/ 下也正確）；首頁由 Logo 返回，居家知識為文字＋書本圖示次要入口
  - LINE 浮動鈕、加入我們鈕、回頂鈕、手機底部 LINE 預約列；網站不提供公開電話 CTA
...
- **正式網站版本**：`b23c755`（後續文件提交除外）；首頁與四個核心服務頁已完成線上 200／共用 UI 資源驗證。
