...
最後更新：2026-07-12（by Codex）— FRONTEND-CRAFT 第一階段質感精修
...
- **`header.js` 是全站共用核心**（每頁 `<script src="header.js">` 或 `../header.js` 載入），runtime 注入：
  - fixed header＋4 個主服務頁籤（root 絕對路徑 `/xxx.html`，讓 /articles/ 下也正確）；桌機 Logo 首頁入口與四頁籤同列等高，手機維持 Logo＋2×2 服務選項
  - 右側 LINE 浮動鈕、回頂鈕、手機底部 LINE 預約列；網站不提供公開電話 CTA，也不顯示「加入我們」
  - 四服務專屬色系由 body theme class 與 CSS variables 串接頁籤及頁面 CTA：冷氣青藍、洗衣機紫、居家清潔琥珀、漏水青綠
...
### 檔案地圖
```
index.html                     首頁（hero、四服務卡、地區條、CTA）
...
- **正式分支**：`main`；2026-07-11 由 `agent/rwd-navigation-unify` 快轉部署四大服務與 RWD 版本。
...
- 新增 **`docs/FRONTEND-REVIEW-2026-07-16.md`**：四個獨立專業視角 AI agent（視覺設計／轉換率 CRO／內容與資訊架構／UX 效能無障礙）平行審 `origin/main` 正式版，去重合併後依「嚴重×成效×成本×跨視角共識」做綜合修改排序（P0 快贏／P1 信任地基／P2 內容對等／P3 精修，共約 30 項）。
- **四視角一致結論**：流量進得來、卡在「最後一步」——低摩擦入口缺失（表單門檻/手機無一鍵 LINE）＋信任地基薄弱（全站零評價、清洗頁零保固）＋品牌換血只換一半（新頁自營、深層頁仍舊媒合平台，敘事矛盾）＋四色品牌系統沒被執行＋手機首屏又擠又慢。
- **業主定案兩決策（全站文案唯一準據，見審查文件「業主已定案」段）**：
  - **決策 A＝混合式**：清潔/家電由灰汰郎安排團隊、漏水以 FAQ 後媒合廠商；款項灰汰郎收、**驗收成功才付廠商**（當客戶保障賣點）；純諮詢免費不施工不收費，**車馬費只在「到場無法施作」收**（冷氣缺型號/照片、洗衣機螺絲鏽死拆不開、水管檢測既有滲漏客戶不洗）；清掉舊「車馬費代管/平台/$500-2000 分級」語，金額與後端 bot 一致（$500）。
  - **決策 B＝保固**：清洗前正常、清洗後無法運作→灰汰郎負責維修；完工兩週內同問題由負責部門免費重洗＋上門檢修（帶「因清洗導致」邊界，與後端知識庫一致）。
- **交接 Codex**：見 §7「前台優化執行（Codex）」。此為唯讀審查，未改任何前台檔案。

### 2026-07-15（Claude・洗洋洋合作意向書 + 談判確認清單）
- 新增 **`docs/PARTNERSHIP-LOI-SEEYANGYANG.md`**：今天與洗洋洋當面談合作用。含①談判前逐項確認清單（服務範圍/第一順位界線、拒絕才轉單的可操作定義、定價對齊、拆帳方案、責任分工、互惠對價、期間終止）②可直接簽的《合作意向書》草案。
- 業主指定新增兩條核心條款：**第三條**甲方五年內不自建施工團隊；**第四條**洗洋洋範圍內品項第一順位、僅於洗洋洋拒絕（明確不接／○小時未回應／無法在客戶要求時間到場）方可轉派，且轉派後不退回。並補互惠條款（乙方不繞單/產能保證/價格穩定）。
- 提醒：唯一未定大項＝拆帳（固定介紹費 vs 抽成%＋數字）；居家清潔走洗洋洋或潔美淨需當面釐清；漏水不在洗洋洋範圍。

### 2026-07-15（Claude・Google Ads 定稿建置指令書）
- 定稿 **`docs/GOOGLE-ADS-BUILD-SPEC.md`**：明天可直接貼給「瀏覽器版 Claude」照設的成品。依業主實際預算 **NT$4,000／月（≈NT$130/日）**重算，5 個活動涵蓋全部服務、居家清潔減弱——A 漏水抓漏 NT$60、B 家電清洗 NT$40、C 水塔水管清洗 NT$15、D 品牌 NT$8、E 居家清潔 NT$8（月 ≈ NT$3,980）。含完整關鍵字、否定詞（共用清單）、出價/出價上限、轉換追蹤（`quote_submit` 主要、`line_click` 次要、auto-tagging、暫用 TWD 500 lead value）、Sitelinks/Callouts/Snippets、執行順序與上線檢查表。
- **RSA 文案全數重新驗證中文字數**（標題 ≤30 單位、說明 ≤90 單位、CJK=2）：82 條標題、23 條說明全部合格（標題最長 17/30、說明最長 82/90）。
- 到達網址全部對照 `origin/main` 實際頁面確認存在（含 Codex 已上線的 `water-tank.html`、`pipe-cleaning.html`），網域 `leakdoctor.tw` 已對照 CNAME/canonical。
- 兩份計畫評比與定案（Codex 主導、併入預算重算與居家減弱）見 **`docs/GOOGLE-ADS-DECISION.md`**；Codex 版 `docs/GOOGLE-ADS-PLAN.md` 為正本、舊 Claude PLAN 已標記被取代。

### 2026-07-12（Claude・Google Ads 建置計畫 + 後端交接補記）
- 新增 **`docs/GOOGLE-ADS-PLAN.md`**：Google 廣告的評估與建置藍圖，含現況盤點、目標/KPI、帳戶結構、**轉換追蹤地基**（填真實 GA4 ID、`quote_submit`/`line_click` 標關鍵事件並匯入 Ads）、關鍵字/否定詞、RSA 文案骨架、到達頁策略、預算出價、上線檢查表，以及 **Claude/Codex/業主 分工表**（業主管帳號金流、Claude 管策略關鍵字文案追蹤法遵、Codex 管落地頁與精美素材）。§7 待辦已改指向此文件。
- `docs/BOT-PLAN.md` 追加「後端進度補記」：P2 夥伴協作流、一連串客訴修正、主模型升 Sonnet 5、戰情室、以及**施工廠商管理＋漏水手動派工**（漏水單進待派工佇列、依縣市推薦、點擊通知夥伴 LINE；技能只針對科技抓漏工具與漏水修補）。權威後端文件仍為 bot repo 的 `AI-README.md`。
...
- [ ] **前台優化執行（Codex）— 依 `docs/FRONTEND-REVIEW-2026-07-16.md` 排序**（四視角綜合審查，已含業主定案的收費/保固口徑）。建議節奏＝先 P0 快贏 → P1 信任地基 → P2 內容對等 → P3 精修：
  - **P0 立即快贏（低成本高成效）**：① 預約表單「地址」降為選填/市區級、步進子表單預設收合（`header.js`）② 手機補「一鍵直達 LINE」雙鈕（現在底部列點下去是開表單）③ 首屏改雙 CTA（加 LINE 免費初判＋填表）④ 修手機 header 吃首屏＋spacer 130→~168px 消跳動 ⑤ 服務頁主 CTA/價格改吃專屬色＋修 `craft.css` 首頁六 icon 只給 4 色的掉色 bug。**審查文件附錄有 P0 五項的「實作建議草圖（參考用，非要求，含大約行號）」——Codex 可參考評估、採不採用/怎麼做自行決定；business 口徑（決策 A/B）才是必須遵循的準據。**
  - **P1 信任地基**：① 補真實顧客評價/星等（全站零）② 每個清洗頁加保固區塊（決策 B 文案）③ 統一商業模式敘事（決策 A＝混合式：對客一律灰汰郎、款項灰汰郎收驗收才付廠商當賣點、車馬費只在到場無法施作收、清掉 about/地區頁/漏水文章的舊平台+分級金額語）④ hero 信任膠囊＋全站補「可開發票」⑤ 首屏效能（`process-images.css` 420KB base64 改真 img、hero `fetchpriority`、header.js 評估 defer）
  - **P2 內容對等**：cases/team/knowledge 補清潔素材（現 100% 漏水）、首屏價值主張、導覽補百科/案例入口、水塔/水管補價錨+案例、漏水文章接 `ldOpenQuote('leak-repair')` 統一漏斗、地理統一八縣市
  - **P3 精修**：modal focus trap、送出 loading/錯誤聚焦、觸控≥44px、四色收斂、照片調色一致、資訊圖用專屬色、比價錨定、季節急迫感、about 補改名敘事、清死碼（舊藍主題/重複 CSS）、電話驗證。完整表格與行號在審查文件。
- [ ] **視覺改版 P2**：延伸至居家清潔、地區、案例、百科與文章頁；建立真實案例／流程示意的圖片標示規格
- [ ] **Google Ads 建置**：**定稿指令書 `docs/GOOGLE-ADS-BUILD-SPEC.md`（可直接照做）**，兩份計畫評比定案見 `docs/GOOGLE-ADS-DECISION.md`。GA4 `G-1H1X1X9QZE` 已上線；動工前唯一前提＝把 `quote_submit`/`line_click` 標關鍵事件並匯入 Ads 轉換、開 auto-tagging。業主明天貼給瀏覽器版 Claude 照設；Codex 主導素材、Claude 版負責預算/文案/追蹤對照。
