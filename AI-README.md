...
- [ ] **Google Ads 建置**：完整計畫與 Claude/Codex/業主分工見 **`docs/GOOGLE-ADS-PLAN.md`**。第 0 階段先補追蹤地基——取得真實 GA4 ID 填入 `header.js`（3 處佔位）、把 `quote_submit`/`line_click` 標為關鍵事件並匯入 Ads 轉換——再由 Codex 做漏水/冷氣兩張轉換型落地頁。
