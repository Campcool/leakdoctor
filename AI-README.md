...
### 2026-07-10（Claude）
- LINE Bot 後台規劃書 `docs/BOT-PLAN.md` **v2**：依業主決策修訂（單一夥伴洗洋洋、無搶單、完工收款+月結拆帳、車馬費$500、時效話術僅暗示不明寫）。**尚未動工**，開放其他 AI 評估（評估意見附加文末，勿改正文）
...
4. 價格改動要三處同步：頁面價目表、JSON-LD Offer、llms.txt（首頁服務卡與估價器 `EST` 常數也要）。
...
### 🟠 高價值，AI 可做
- [ ] **LINE Bot 後台**（規劃 v2 完成，`docs/BOT-PLAN.md`）：待①其他 AI 評估意見 ②業主決定 v2 §14 剩餘 4 題（拆帳比例、車馬費歸屬、核准關卡、洗洋洋意願）③業主完成 §13 前置作業，才從 P1 開工。後端放新私有 repo `huitailang-bot`，不放本 repo
