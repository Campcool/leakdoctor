...
最後更新：2026-09-01（Codex）— 六服務「深入了解」卡片分層修正、窗型冷氣一次 3 台以上才承接；服務 hover 子選單與知識文章導覽均保留。

## 2026-08-31 浮動聯絡區重修 ＋ DESIGN.md 對齊（Claude，**業主授權直接合併 main**）

### 授權與範圍

業主 2026-08-31 明確指示「不看，直接 main 上去，交接文件寫清楚即可」。
因此本批**未開 PR，由 Claude 直接 fast-forward 合併 main 並推送**，
合併前 `origin/main` 為 `6f700d7`、無其他人的新 commit，屬乾淨快進。

合併的三筆（皆在 `claude/cleaning-service-search-scope-j8v2td`）：

| commit | 內容 | 是否動到畫面 |
|---|---|---|
| `d0a7586` | 給 Codex 的唯讀審查請求文件 | 否（純文件） |
| `d57785a` | **浮動聯絡區重修** | **是**（`header.js`、`assets/uiux-polish.css`） |
| `d92b3c4` | DESIGN.md 對齊現行實作 | 否（純文件） |

### 為什麼要修浮動聯絡區

Codex `6f700d7` 把 LINE 與填單估價包進一張白卡片。實測問題：

1. **三層圓角疊在 100px 寬容器裡**：白卡片(半徑 20px) → 白色 LINE 方塊(14px) → 深藍鈕(12px)。
   LINE 方塊與外層同為白底，邊界看不見，視覺上變成「綠色圖示浮在白框裡」。
   違反 `DESIGN.md` §6「同一區塊最多一層卡片嵌套」。
2. **兩個實色互相競爭**：LINE 綠與品牌深藍同時填滿。
   違反 `DESIGN.md` §3「單一畫面最多一個主 CTA 色；LINE 綠只代表開啟 LINE」。
3. 86px 寬硬塞「填單估價」＋「先填需求」兩行；兩顆等重動作垂直並列，沒有層級。

### 改法（現行生效版本）

- 外層 `#ld-contact-island` **只當版面用**：`background:none;border:0;padding:0;box-shadow:none`。
  再加回底色／邊框／陰影就會變回卡片包卡片，已有測試擋住。
- **LINE 為主操作**：官方 PNG 直接當按鈕表面（60×60），容器 `background:none`，
  消除「白框包綠框」；陰影用 LINE 綠低透明度。仍不另疊 LINE 字樣。
- **填單估價為次操作**：白底細框。2026-08-31 業主追加要求「文字拆兩行、長寬與下方 LINE 一致」，
  故由原本的 94×44 藥丸改為 **60×60、圓角 18px**，與 LINE 鈕同寬同高同圓角、右緣對齊，
  文字拆成「填單／估價」兩行；副標仍只存在於 `aria-label`。
- **DOM 順序改為估價鈕在前、LINE 在後**，讓 Tab 順序＝視覺順序。
- `uiux-polish.css` 版本號更新為 `?v=20260831d`（避免瀏覽器吃到舊 CSS）。

量測：全區僅一個實色；文字對比 **13.13:1**；觸控 94×44 與 60×60；
1024/1280/1440/1920px 皆不與底部固定列或回頂鈕重疊、不超出畫面；
≤1023px 維持隱藏由底部固定列接手；`prefers-reduced-motion` 停用位移。

### DESIGN.md 改了什麼（規則跟著實作走）

`6f700d7` 新增全站廠商招募入口並讓浮動區變成兩個動作，**但沒有同步 DESIGN.md**，
與兩條既有條文牴觸。業主確認保留現況，因此改規則而非改程式：

- §6 舊條文「公開前台不顯示『加入我們』合作招募入口，主要浮動操作只保留 LINE」**已刪除**，
  改為：招募入口只能在該頁**最後一個轉換 CTA 之後、`<footer>` 正上方**，
  由 `header.js` 統一注入指向 `/join.html`；深底單行文字連結，
  **不得做成主 CTA 按鈕、不得上移到 Hero／價格區／案例區**。
- §6 新增浮動區條文：LINE 主、填單估價次、單一實色、外層不得成為卡片。
- §7 廣告落地頁：目前**沒有獨立落地頁**，Google Ads 直接導向首頁與六個服務頁，
  故同一條規則適用於這些頁。

### ⚠️ 留給下一棒的數字

招募入口與該頁最後一個轉換 CTA 的實測間距：
...
首頁的招募入口緊接在 `#cta-bottom` 下方。本輪未調整（業主選擇保留現況）。
**若日後發現首頁轉換率受影響，優先調整的是這段間距，不是把入口整個拿掉。**

### 驗證

`node --check header.js`；`node --test` 四支共 **106/106**（Header 契約由 15 → 23 案）；
`validate-site.mjs` 23 頁／34 sitemap；`check-home-bot-contract.mjs` 29 項綠。

**突變驗證（改壞真實檔案，非 in-memory）九個全部被攔截**：
外層加回白底卡片、LINE 鈕套回白底、次操作改回深藍實色、估價鈕加回第二行副標、
DOM 順序改回 LINE 在前、招募入口改插頁首、招募入口套主 CTA 品牌橘、
DESIGN.md 改回舊條文、拿掉 DESIGN.md 的單一實色規則。

新增測試中有一條是**「DESIGN.md 與實作同步」檢查**——這批的問題根源就是
改了實作沒改文件，所以讓它變成會紅的東西：舊條文只要還在就失敗。

### 未驗證（不得宣稱通過）

- iOS／Android／LINE webview 真機；桌面模擬不等於真機。
- 螢幕閱讀器實測、Lighthouse。
- 浮動區只在 ≥1024px 顯示，**手機使用者看不到這次的視覺改動**。
- Bot A（草稿修訂被去重吞掉）／B（working_lead 非首次轉換才發送）本輪未碰。
...
### 2026-09-01 六服務知識卡片與窗型承接條件修正（Codex）

- 業主截圖確認六個服務頁的「深入了解」卡片都有分類、標題與右側編號疊字。根因是 `assets/craft.css` 的 `body.service-page .knowledge-card{padding:1rem!important}` 權重高於共用新樣式；已改用更精準的 `body.service-page .knowledge-rail .knowledge-card`，固定保留 4rem 上列，分類／編號與標題分層，不以單頁補丁處理。
- 窗型冷氣公開條件統一為「一次 3 台以上才承接」，同步首頁、冷氣服務內容、價格表、FAQ／JSON-LD、冷氣費用文章、OG 摘要、首頁詢價 catalog 與 `data/service-options.json`；數量選擇最小值改為 3。不得再改回「未滿三台先確認」或單台可送出。
- 共用 `header.js` cache key 升為 `20260901c`，37 個 HTML 同步。六個服務頁均實際載入 1 個知識區、每區 3 卡；以 1440×900 與 390×844 截圖確認無疊字。`validate-site.mjs`、header contract 25 項與首頁服務 37 項均通過。
...
- 共用 `header.js` cache key 初版為 `20260901a`（後續卡片修正已升至 `20260901c`），37 個 HTML 引用同步更新；保留 2026-08-31 後的價格、合作申請、OG、無障礙與首頁箭頭流程更新。本機 `validate-site.mjs` 23 頁全綠；header、閱讀節奏、首頁服務、流程四套契約共 121 項通過。16 篇文章各只有一套新版目錄，並以 1440×900／390×844 實際檢查服務頁、百科與文章。兩個 mutation harness 的基準均通過，但手機導覽有 5 個、modal 有 1 個「預期攔截／放行」測試仍不一致，屬門禁測試精度待另案處理，不是目前正式 baseline 失敗。

### 2026-09-01 首頁服務安排箭頭流程（Codex）

- 業主指出首頁 Hero 下方「先確認／明碼標價／後安排」三段散落圓點不好看；改為語意化 ol 的 01／02／03 流程，使用橘色細框步驟圓與箭頭串接。
- 桌機維持三欄橫向閱讀，759px 以下改為單欄及向下箭頭；文案縮短但保留原本界線：常見項目直接標價、特殊條件確認後才安排可承接團隊。
- 僅改 index.html 與 assets/home-refresh.css，並 bump 首頁 CSS cache key；未改價格、表單、服務邏輯、OG、canonical、Bot 或 LINE 發送。
- 本機 129／129 測試通過；以 1265px 桌機及 390×844 手機實際截圖檢查，三步順序、箭頭、換行與周邊版面正常。正式部署與真機狀態另以 main／Actions／正式頁覆驗為準。

### 2026-08-31 全站分頁 OG 圖（Codex，整批生成及發布獲業主授權）

- 34 個新增主題各生成四版，共 136 張候選，檢視全幅及中央方形縮圖後選版。全站 37 頁各有獨立 OG／Twitter 圖；join 保留業主選定的正式商務 A 圖。
- 案例與團隊頁沿用既有去識別照片、原始 bytes 不變；未把 AI 情境冒充真實施作、人像或地標。來源、選版與提示詞見 scripts/og-manifest.json，規格見 docs/OG-BATCH-20260831.md。
- 補齊初始 HTML 的圖片、尺寸、MIME、alt、分頁標題與 Twitter metadata；不改正文、價格、canonical、robots、CSS、JS、favicon 或 Bot。
- 新增 OG 契約與五種隔離 mutation 檢查，納入 Pages CI。部署狀態以 Actions 與正式 HTTP／圖片 SHA256 覆驗為準；LINE 舊訊息快取不保證即時更新。
...
- 業主最新決定取代 R2「每頁至少兩個中段 CTA」：估價集中桌機 LINE 浮島下方，手機沿用底部操作列；首頁的互動價格試算仍保留，所有頁面／文章重複的 ldOpenQuote 按鈕移除。共用表單、價格、LINE 契約不變。
- 流程右欄依實際高度垂直置中（761px 以上）；手機維持上下排列，details 展開與24px文字不裁切。首頁服務原則改標題／說明分行，兩個價格入口補完整按鈕樣式及容器對齊。
- 新增 join.html：廠商／團隊、統編選填、據點、聯絡方式、22縣市、六大服務展開小項、設備／證照、發票／收據、保險及資料使用同意。選項直接讀取後台公開能力目錄，不含成本；頁尾新增加入我們入口。
- POST /api/partner-applications 寫獨立 D1 申請表。未取得成功回覆不宣稱已存檔；同次重試保留 requestId，不自動建正式廠商或推播 LINE。
- 已驗證：前台100項、23頁／34 sitemap網址；後台333項含隔離 D1 併發去重／權限／實際存檔。瀏覽器8頁×4實際寬度（375/768/1024/1440）無水平溢出；本機表單→後台查看→更新備註成功。正式部署結果另以 Actions 與發布報告為準。


### 2026-08-31 main 發布授權與候選確認

- **正式結果（11:22，Asia/Taipei）**：功能候選 `c8e30cc` 已推main，Pages Actions `33353522866` 成功。正式 `water-tank.html` 已看到4499／5999／9999三級距與NT$1,500個案討論提醒；正式共用預約表單亦已揭露。Bot已部署、D1 0015～0017成功、health／ready正常。以下歷史段落的「尚未推main／部署」由本條取代。
- 額外兩組mutation runner會修改同一header.js，須循序執行：`node --test --test-concurrency=1 scripts/test-*.mjs`；100/100通過（包含13個手機導覽與20個modal mutation案例），工作樹未留測試注入。

- 業主已授權完成後發布 main，由業主接續雙平台人工檢查。以真實 main `c7cc1d6` 整合已審查的閱讀節奏、水泥三级距、價格／modal契約與車馬費揭露，沒有強制覆寫遠端。
- 候選重新通過98項前台測試、22頁／33網址結構驗證、29品項及三個水泥級距跨Bot契約；Bot323項通過。發布順序為Bot與D1先完成，再發布前台。

### 2026-08-31 水男孩白跑費用（業主最新定案，取代先前待決註記）
- 對客揭露：若由水男孩企業社承接，師傅到場後因現場條件無法施作時，可能涉及 NT$1,500 車馬費；實際是否收取及處理方式，先由專員、施工廠商與客戶依個案討論確認，並非一律加收。
- 內部規則：水男孩白跑先個案討論。車馬費基準 NT$1,500；未能向客戶收取的部分由水男孩自行吸收，灰汰郎不代墊、不補貼，不自動產生應付水男孩款項。
- 網站首頁試算、水塔／水管價目、漏水FAQ及共用預約送出前提醒同步。Bot FAQ／費用工具同步，0017只更新未客製預設回答。
- 水男孩無法施作／舊收費按鈕轉通知業主個案處理，不自動按500或1500結案；其他廠商流程不變。實收與結案人工核對，不認列未收到的收入。
- 尚未發布，未更動正式D1或發送正式LINE。
...
- [x] 業主已同意整批設計並授權生成及上架：34 主題四版候選、37 頁獨立分享圖與 OG 契約已整合。
- [ ] 業主在 LINE 重新分享各分頁驗收小圖；服務端圖片可讀不代表既有聊天預覽已刷新。
...
- 正式三級距與費用提醒已核對；下一步為業主雙平台人工檢查。水男孩正式LINE ID仍待於後台綁定，綁定前可询價但不能驗證水男孩自動收件。未代客送單、未發LINE；既有Bot草稿修改／working_lead重複仍另案待修。

- [x] 水男孩可能NT$1,500車馬費與個案討論揭露（本機）。
- [ ] 與水塔修正版協調發布並覆驗正式頁面／Bot。
...
- [x] **DESIGN.md 與實作對齊（2026-08-31，業主選 (a)）**：Codex 的 `6f700d7` 新增全站
      「加入我們 · 廠商合作申請」入口並讓浮動區變成兩個動作，但沒有更新 `DESIGN.md`，
      與 §6「公開前台不顯示加入我們入口，主要浮動操作只保留 LINE」牴觸。
      業主確認保留現況，故改寫 §6 該條與 §7 廣告落地頁條文：招募入口只能在
      最後一個轉換 CTA 之後、`footer` 正上方，不得做成主 CTA 或上移到內容區；
      浮動區為「LINE 主、填單估價次」、全區單一實色。新增 4 條測試（含 DESIGN.md 與實作同步檢查）
      與 4 個突變。實測招募入口位於 91–92% 捲動深度；**首頁距最終 CTA 僅 158px，
      是全站最近的一處**（服務頁 398–566px），若日後發現首頁轉換受影響，優先調整間距。
- [x] **客戶端閱讀節奏三件套（2026-08-31 Claude，借鏡 SimplyCarbs /business）**：
      六個服務頁 hero 之後加 ① Key Impact 三個結論式數字 ② 證據列（真實案例縮圖前移，
      從 60–90% 捲動深度提到 **9–26%**）③ 客戶時程軸四步（放在價格區前）；
      FAQ 標題改為點名居住情境。**48H／72H 一律附兩層但書**（業主指示：旺季擠不出來、
      客戶可能要求夜間配合），不得寫成保證。規則見 `DESIGN.md` §12-A。
      新增 12 條測試＋7 個突變；30 組尺寸／根字級量測零溢出、零 <12px、零觸控 <44px、零破圖。
- [x] **浮動聯絡區重修（2026-08-31 Claude）**：Codex 的 `6f700d7` 把 LINE 與填單估價包進一張
      白卡片，形成「白卡片→白方塊→深藍鈕」三層圓角（半徑 20/14/12）＋兩個實色互相競爭，
      違反 `DESIGN.md` §3「單一畫面最多一個主 CTA 色」與 §6「同一區塊最多一層卡片嵌套」。
      改為：拿掉外層卡片，官方 LINE PNG 直接當按鈕表面（不再套白底造成雙框，60×60），
      填單估價降為白底細框藥丸（94×44、單行、副標移到 aria-label），DOM 順序＝視覺順序＝Tab 順序。
      全區只剩 LINE 綠一個實色；文字對比 13.13:1；1024–1920px 皆不與底部列或回頂鈕重疊。
      新增 4 條 Header 契約測試＋5 個突變。
- [x] **價格試算改品項抽屜（2026-08-31）**：六大項（冷氣／洗衣機／居家／水塔／水管／補漏），
      細項預設只露出一項最常見的（冷氣＝壁掛室內機、洗衣機＝直立式、居家＝定時 4 小時；
      水塔／水管／補漏不展開），按大項的＋才展開其餘；**收合時已選項目仍會顯示**，
      否則客戶會看不到自己選過什麼、總價卻在跳。新增黏底醒目總價列（手機 28px、桌機 32px）。
      水泥水塔開賣：三級距 **4,499／5,999／9,999**（業主 2026-08-31 定價，一律成本 + 999 固定毛利，
      不是百分比加成），100 噸以上現場勘查；
      另加四項給／排水漏水施工（皆待報價，金額級距高不進即時試算）。
      前台品項 21→29，跨 repo contract 同步。**cat 是畫面分類，不可動 Bot 用的 group。**
- [x] **閱讀節奏強化 R1／R2／R3／R5 已實作（2026-08-31 Claude）**：
      R1 首屏補起價（七頁首個價格深度 16–26%／無 → **3–6%**）；
      R2 中段補 15 顆 CTA（桌機最大空窗 39–80% → **25–36%**）；
      R3 五個服務頁各補 3 題異議處理並排在衛教題前（首頁 Codex 已補 3 題）；
      R5 居家清潔補痛點開場（無 → 15%）、水塔痛點 63% → 37%。
      **R4 水管價格口徑仍待業主裁決**（首頁 $3,599 vs 水管頁無數字），未動。
      新增 `scripts/test-reading-rhythm.mjs`（12 案含 4 突變）。
- [ ] ~~閱讀節奏強化（R1–R5）~~ 原始交接：：首屏零數字／價格藏在點擊後、桌機 CTA 中段空窗 39–80%、
