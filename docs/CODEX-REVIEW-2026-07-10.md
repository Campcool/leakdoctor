...
### 補充：FAQ 資料庫 vs 直接串 AI（2026-07-10 09:03 Asia/Taipei）

業主提出的疑問是正確的：如果後台只是 FAQ 資料庫，回答會穩但呆板；如果直接把 LINE webhook 串 ChatGPT/Claude 自由回答，回答會自然但可能亂講價格、服務範圍或承諾。

Codex 建議不要選「純 FAQ」或「純 AI」，而是採用 **AI + D1 資料庫 + 固定規則工具**：

```text
LINE 使用者
  -> LINE Webhook
  -> Cloudflare Workers 驗簽與寫入事件
  -> AI 判斷意圖與抽取欄位
  -> 呼叫工具查 D1 / config / pricing
  -> AI 只根據查到的資料產生自然語氣
  -> LINE reply 或 push
```
...
getFaqAnswer(topic)
createDraftOrder(customer, service, detail, preferredTime)
requestHumanHandoff(reason)
```

這樣可以同時解決兩個問題：

- 不像 FAQ 那麼死板：客戶用自然語言描述「新北兩台冷氣，一台很臭一台不冷」時，AI 可以理解並拆成欄位。
- 不讓 AI 亂講：價格、車馬費、服務範圍、預約規則都必須從 D1 或設定檔取得。

具體決策建議：

1. P1 webhook 可以直接串 Claude 或 ChatGPT，但 system prompt 必須明確禁止模型自行編價格、保固、施工時間。
2. 服務價格與規則不可只寫在 prompt，必須存在 D1 / `services.json` / `config`，並由工具查詢。
3. AI 回覆分兩類：
   - `direct_answer`：查資料後可直接回覆。
   - `handoff_required`：價格不明、漏水複雜案、客訴、殺價、特殊時段，轉人工。
4. 所有 AI 判斷結果都存入 `events`，包含 intent、confidence、tool calls、final reply。
5. 如果 AI tool call 失敗，回覆應該降級成「我先幫你登記，稍後由專人確認」，不要硬答。

給 Claude 後續討論的問題：

- P1 要選 Claude 還是 ChatGPT 作為主要客服模型？或先做 provider adapter，保留替換能力？
- 價格與 FAQ 的單一來源要放在 bot repo 的 JSON config，還是直接放 D1 後台可編輯？
- 哪些情境一定轉人工？建議先列成 `handoff_rules`，不要只靠 AI 自行判斷。

## 建議執行順序

1. 網站先補 GA4、About schema、圖片尺寸、全站估價流程。
2. 同步建立後台私有 repo `Campcool/huitailang-bot`。
3. P1 bot 只做：LINE webhook、AI 意圖判斷、工具查價、客戶建單、老闆通知、合作方通知、狀態按鈕、D1 event log。
