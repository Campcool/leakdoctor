...
## 後端進度補記（by Claude, 2026-07-12 Asia/Taipei）

> 完整技術細節與版本表見 **bot repo `Campcool/leakdoctor-bot` 的 `AI-README.md`**（唯一權威後端交接文件）。此處只留跨 repo 摘要，方便前台/文件端掌握。

- **P2 夥伴協作流上線**：夥伴訂單卡三鈕（已約定時間/無法施作/備註）、狀態機推進、cron 提醒（逾時/D-1/完工日詢問/今日施工/老闆日報）。
- **一連串真實客訴修正**：改期真正重發確認卡（禁止 AI 幻想）、確認卡補齊姓名/電話/清洗地點/服務明細/金額、派工必填完整地址（缺就擋）、過期日期驗證、地址縣市偵測、**地址確定性捕捉**（訊息像地址就直接存，不靠 AI 呼叫工具）、**地址＋日期時間同訊息時精準分欄**（日期不再被塞進地址）。
- **主模型升級 Haiku 4.5 → Sonnet 5**（`claude-sonnet-5`）：多輪工具呼叫穩定度大幅提升（老闆回報「太呆」的根因）；同步移除 `temperature`（Sonnet 5/Opus 系列會 400）。
- **戰情室（`/admin`）**：唯讀看板（總覽/月曆/訂單）＋放大置中＋果凍感；業主已設定帳密啟用。
- **施工廠商管理＋漏水手動派工（新）**：冷氣/洗衣機維持自動轉洗洋洋；**漏水/抓漏/補漏**因各地技師不同，客戶確認後進「待派工」佇列，由業主在戰情室依「客戶縣市」推薦手動挑夥伴、點擊自動通知對方 LINE。夥伴技能只針對**科技抓漏工具**（熱像儀/內視鏡/聽漏/水分儀/打壓/透地雷達/追蹤氣體）與**漏水修補**（打針高壓灌注/防水/壁癌/管線更換）。戰情室新增「派工」「廠商」兩分頁（可寫，沿用 Basic Auth）。
- **服務目錄補齊**：新增**水管清洗**與**抽油煙機清洗**（歸居家清潔），皆現場報價；補齊洗洋洋既有品項。
- **AI 知識庫（可後台編輯）**：常見問答由 bot 的 `getFaqAnswer` 回答，改為老闆可在戰情室「知識庫」分頁自行新增/編輯/停用、即時生效（D1 規則優先、程式內建 fallback）；含命中統計與「未命中」清單、測試器。內容只用官網已發佈 FAQ＋catalog，不自創價/承諾。
- **手動接管／靜音**：老闆想親自處理某客戶時可逐位靜音 bot（仍記錄不回覆），或全域暫停 AI（頂端紅色橫幅提醒）。
- **完工後自動邀評**：夥伴回報完工 → 私訊客戶感謝＋Google 評論按鈕（連結由戰情室「設定」貼上）；直接餵養 SEO 與 Google Ads 信任分數。
- **系統錯誤紀錄**：Worker 錯誤持久化並去重，戰情室「總覽→系統狀態」一目了然。
- **與行銷的接點**：漏水抓漏是舊品牌核心、也是 Google Ads 首波投放重點之一；廣告進線的漏水單會自然落入「待派工」流程，完工後自動邀評又回補評論。投廣告前的網站/追蹤待辦見 `docs/GOOGLE-ADS-PLAN.md`；GA4 ID `G-1H1X1X9QZE` 待填入 `header.js`（Codex）。
- **這幾項參考自業主另一個 bot `Campcool/campcool-bot`（露涼社）的做法再優化**，不照抄；權威技術細節一律以 bot repo `AI-README.md` 為準。
